...
public class ManageProductsFrame extends JFrame implements ActionListener {
    ...
    private final JLabel idLabel, nameLabel, priceLabel, categoryLabel, instructionLabel, imageLabel;
    private final JTextField idTextField, nameTextField, priceTextField, imageTextField;
    ...
    ManageProductsFrame() throws SQLException{
        ...
        categoriesArray = new String[]{"Cookware", "Kitchen Tools", "Storages"};
    ...
    private void processImageInput(Product product) {
        if (!imageTextField.getText().isBlank()) {
            product.setImage("images/products/" + imageTextField.getText());
        }
    }

    private void dbCreateProduct(Product product) {
        processImageInput(product);
        if (isBoxesEmpty()) {
